...
"Please enter your password.")+"</p></div>").appendTo(c),e=a('<div class="passwordwrapper"/>').appendTo(b),b=a('<div class="buttons right"/>').appendTo(b),d=a('<div class="btn">'+Craft.t("Cancel")+"</div>").appendTo(b);a('<input type="submit" class="btn submit" value="'+Craft.t("Continue")+'" />').appendTo(b);this.$currentPasswordInput=a('<input type="password" class="text password fullwidth"/>').appendTo(e);this.$spinner=a('<div class="spinner hidden"/>').appendTo(b);this.modal=new Garnish.Modal(c);
...
{password:b},a.proxy(function(c,d){this.$spinner.addClass("hidden");"success"==d&&(c.success?(a('<input type="hidden" name="password" value="'+b+'"/>').appendTo("#userform"),a("#email, #newPassword").removeClass("disabled").removeAttr("disabled"),this.$lockBtns.remove(),this.modal.hide()):Garnish.shake(this.modal.$container))},this)))}})})(jQuery);
